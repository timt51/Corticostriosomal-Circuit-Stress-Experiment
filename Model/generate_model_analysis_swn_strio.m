...
fig_dir = [ROOT_DIR 'Model Reduced SWN Strio Connectivity/Striosomes/'];
if ~exist(fig_dir, 'dir')
    mkdir(fig_dir);
end
saveas(fs(1), [fig_dir 'Control Striosomes Line Plot'], 'fig');
saveas(fs(1), [fig_dir 'Control Striosomes Line Plot'], 'epsc2');
saveas(fs(1), [fig_dir 'Control Striosomes Line Plot'], 'jpg');
...
disp([window_starts{neuron_type_idx}]);
fig_dir = [ROOT_DIR 'Model Reduced SWN Strio Connectivity/SWNs/'];
if ~exist(fig_dir, 'dir')
    mkdir(fig_dir);
end
...
save('neuron_type_zscores_model_swn_strio.mat', 'all_zscores');

all_times_of_max_model = all_times_of_max;
save('all_times_of_max_model_swn_strio.mat', 'all_times_of_max_model');
%% Cascade
f = figure;
count = 3;
...
fig_dir = [ROOT_DIR 'Model Reduced SWN Strio Connectivity/'];
if ~exist(fig_dir,'dir')
    mkdir(fig_dir);
end
