function [plotting_bins, evt_times_distribution, timeOfBins, numTrials, fullData] = ah_fill_burst_spike_plotting_bins(large_bursts_array, large_burst_spikes_array, ses_evt_timings, neuron_idsAndData, selection_criteria, selected_trials, plotting_parameters, normalization_type, thresholds, keep_allData)
%AH_FILL_BURST_SPIKE_PLOTTING_BINS creates bins that can be plotted by either
...
%Inputs are:
% LARGE_BURSTS_ARRAY - a cell array where each entry is a list of the
%   bursts within a given window in that trial. Trials must be in order
...
% LARGE_BURST_SPIKES_ARRAY - a cell array where each entry is a list of the
%   spikes within a given window in that trial. Trials must be in order
%   within their neuron, and clumped together with trials for neuron 1
%   ahead of trials for neuron 2 if and only if neuron 1 is indexed before
%   neuron 2. Otherwise this function won't work and might run into errors.
%   Note that this array contains only spikes that participate in a burst
...
if keep_allData && neuron_normalization
    fullData = -ones(size(neuron_idsAndData,1),numBins);
elseif keep_allData
    fullData = -ones(length(large_bursts_array),numBins);
end
for trial_idx = 1:length(large_bursts_array)    %loop through every trial
    ...
    if ~skip_trial   %if we updated skip_trial, then we want to skip the trial, so we skip update of tmp_bins
        ...
        if ~(t_second_align_event - t_first_align_event <= 0 || t_second_align_event - t_first_align_event > 5) %Removing bad trials
            ...
            num_spikes = length(large_burst_spikes_array{trial_idx});   %number of spikes participating in a burst in given trial
            spike_idx = 1;
            for burst_idx = 1:num_bursts                        %loop through bursts of trial
                ...
                while spike_idx <= num_spikes && large_burst_spikes_array{trial_idx}(spike_idx) <= burst_end_time
                    spike_time = large_burst_spikes_array{trial_idx}(spike_idx); %time of spike
                    if spike_time < burst_start_time
    ...
    if sum(tmp_bins)    %to skip case in which there are no spikes in the trial
        ...
        if keep_allData && neuron_normalization
            fullData(current_neuron,:) = tmp_bins;
        elseif keep_allData
            fullData(trial_idx,:) = tmp_bins;
        end
