function [zscore, stderr] = ah_plot_double_aligned_population_analysis(plotting_bins, evt_times_distribution, timeOfBins, numTrials, event_columns, extra_event_locs, smoothing_coeff, fig_marker, color, index1, index3)
...
% [~, index1] = min(abs(timeOfBins - first_align_evt_time));
% [~, index2] = min(abs(timeOfBins - second_align_evt_time));
% [~, index3] = min(abs(timeOfBins - event_time));

BLmean = mean(plot_bins(60:240));
BLstd = std(plot_bins(60:240));
minimum = min(plot_bins(index1:index3));

% subplot(1,2,1);
% plot(timeOfBins,plot_bins,'LineWidth', 1.5, 'Color', [.5 .5 .5]);
% line([first_align_evt_time, first_align_evt_time], [min_bin max_bin], 'Color', 'Black', 'LineWidth', 2)     %line at first align event time
% line([second_align_evt_time, second_align_evt_time], [min_bin max_bin], 'Color', 'Black', 'LineWidth', 2)   %line at second align event time
% line([event_time event_time], [min_bin max_bin], 'Color', 'Black', 'LineWidth', 2)  %line of time of event

plot_bins = (plot_bins - BLmean) / BLstd;

% subplot(1,2,2);
% plot(plot_bins,'LineWidth', 1.5, 'Color', [0 0 0]);
% line([first_align_evt_time, first_align_evt_time], [min_bin max_bin], 'Color', 'Black', 'LineWidth', 2)     %line at first align event time
% line([second_align_evt_time, second_align_evt_time], [min_bin max_bin], 'Color', 'Black', 'LineWidth', 2)   %line at second align event time
% line([event_time event_time], [min_bin max_bin], 'Color', 'Black', 'LineWidth', 2)  %line of time of event

if isnan(index3) || index3 < 10
   index3 = length(timeOfBins) / 2; 
end

plot_bins = plot_bins(index1:index3);
timeOfBins = timeOfBins(index1:index3);
zscore = mean(plot_bins);
stderr = std(plot_bins)/sqrt(length(plot_bins));

% hold on; scatter(timeOfBins(index3), zscore); hold off;
