...
for db = 1:length(dbs)
    for pair_num = 1:size(hfn_strio_pairs{db},1)
        if isempty(phasic_FR_pairs{db}{pair_num}) || isempty(tonic_FR_pairs{db}{pair_num})
            continue
        end
...
%     mkdir(fig_dir);
% end
% saveas(f, [fig_dir 'Proportion of Inhibited Pairs in Baseline Period'], 'fig');
% saveas(f, [fig_dir 'Proportion of Inhibited Pairs in Baseline Period'], 'epsc2');
% saveas(f, [fig_dir 'Proportion of Inhibited Pairs in Baseline Period'], 'jpg');
...
%     mkdir(fig_dir);
% end
% saveas(f, [fig_dir 'Strength to Strength Comparison in Baseline Period'], 'fig');
% saveas(f, [fig_dir 'Strength to Strength Comparison in Baseline Period'], 'epsc2');
% saveas(f, [fig_dir 'Strength to Strength Comparison in Baseline Period'], 'jpg');