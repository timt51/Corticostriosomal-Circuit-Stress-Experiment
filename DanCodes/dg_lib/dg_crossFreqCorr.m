function [corr, f] = dg_crossFreqCorr(data, varargin)
...
for tempfilenum = 1:numbatches
    ...
    diffs = P - repmat(avgP, 1, size(P,2));
...
if plotflag
    if fs ~= 0
        ...
        % This arcane label refers to harmonics of the reciprocal of the
        % window duration, with DC represented as "Harmonic 0".
        xlabstr = 'Harmonic';
        ylabstr = xlabstr;
    end
    hDisp = figure;
    ...
    titlestr = sprintf( ...
        'Cross-frequency Self-Correlogram, win=%s detrend=%s nw=%.2f k=%.0f, pad=%.0f', ...
        winsizestr, detrendstr, NW, K, pad);
    if ~isempty(descrip)
        titlestr = sprintf('%s\n%s', titlestr, descrip);
    end
    [hI, hCB] = dg_showGram(hDisp, f, f, corr, ...
        titlestr, ...
        xlabstr, ylabstr, 'correlation');
    hCB_new = dg_recolorGram(hCB, [-1 1], hI);
end
