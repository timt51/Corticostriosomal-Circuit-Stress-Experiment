...
% 1 data
pls_spikes{3} = pls_spikes_stress;
strio_spikes{3} = strio_spikes_stress;
swn_spikes{3} = swn_spikes_stress;
Ts{2} = Ts_stress;

pls_spikes = pls_spikes([1,3,2]);
strio_spikes = strio_spikes([1,3,2]);
swn_spikes = swn_spikes([1,3,2]);
Ts = Ts([1,3,2]);
%% Analyze
...
fig_dir = [ROOT_DIR 'Model/Control Input to Stress/Striosomes/'];
if ~exist(fig_dir, 'dir')
    mkdir(fig_dir);
end
saveas(fs(2), [fig_dir 'Control Input to Stress Line Plot'], 'fig');
saveas(fs(2), [fig_dir 'Control Input to Stress Line Plot'], 'epsc2');
saveas(fs(2), [fig_dir 'Control Input to Stress Line Plot'], 'jpg');
saveas(fs(3), [fig_dir 'Control Input to Stress2 Line Plot'], 'fig');
saveas(fs(3), [fig_dir 'Control Input to Stress2 Line Plot'], 'epsc2');
saveas(fs(3), [fig_dir 'Control Input to Stress2 Line Plot'], 'jpg');
...
disp([window_starts{neuron_type_idx}]);
fig_dir = [ROOT_DIR 'Model/Control Input to Stress/SWNs/'];
if ~exist(fig_dir, 'dir')
    mkdir(fig_dir);
end
saveas(fs(2), [fig_dir 'Control Input to Stress Line Plot'], 'fig');
...
save('neuron_type_zscores_control_input_to_stress.mat', 'all_zscores');

all_times_of_max_model = all_times_of_max;
save('all_times_of_max_control_input_to_stress.mat', 'all_times_of_max_model');
...
fig_dir = [ROOT_DIR 'Model/Control Input to Stress/'];
if ~exist(fig_dir,'dir')
    mkdir(fig_dir);
end
...
load('all_times_of_max_control_input_to_stress.mat');
bins = -3:.5:6;
...
ylabel('Time (s)');
title({'Striosomes Timing with Control Input to Stress Models',...
        ['Stress: ttest2 p =' num2str(p2)], ...
        ['Stress2: ttest2 p =' num2str(p3)], ...
        'IGNORE CONTROL BARS'});
fig_dir = [ROOT_DIR 'Model Comparison/Control Input to Stress/'];
if ~exist(fig_dir, 'dir')
    mkdir(fig_dir);
end
...
xlabel('Experimental Group');
ylabel('Time (s)');
title({'SWNs Timing with Control Input to Stress Models',...
        ['Stress: ttest2 p =' num2str(p2)], ...
        ['Stress2: ttest2 p =' num2str(p3)], ...
        'IGNORE CONTROL BARS'});
...
load('neuron_type_zscores_control_input_to_stress.mat');
...
xlabel('Experimental Group');
ylabel('Zscore');
title({'Striosomes Acitivity Zscores with Control Input to Stress Models',...
        ['Stress: ttest2 p =' num2str(p2)], ...
        ['Stress2: ttest2 p =' num2str(p3)],...
        'IGNORE CONTROL BARS'});
fig_dir = [ROOT_DIR 'Model Comparison/Control Input to Stress/'];
