%% Find all PL and SWNs in each session
...
for db = 1:length(dbs)
    ...
    for sessionDir_idx = 1:length(sessionDir_neurons{db})
        ...
        swn_neurons{db}{sessionDir_idx} = intersect(sessionDir_neurons{db}{sessionDir_idx},[cb_swn_ids{db} bbs_swn_ids{db}]);
    end
end
%% Find firing rates associated with each session
swn_activity_types = {'Baseline Firing Rate', 'In Task Zscore', 'In Task Firing Rate'};
pls_activity_type = swn_activity_types{1};
swn_activity_type = swn_activity_types{1};
...
for db = 1:length(dbs)
    for sessionDir_idx = 1:length(pls_neurons{db})
        if ~isempty(pls_neurons{db}{sessionDir_idx}) && ~isempty(swn_neurons{db}{sessionDir_idx})
            ...
            elseif strcmp(pls_activity_type, 'In Task Firing Rate')
...
suptitle({'Parameters: ', ...
        ['At least ' num2str(min_per_session) ' PLs neurons and SWNs per session'], ...
        ['Removing points with SWN firing rate <' num2str(min_swn_FR) ' Hz']});
